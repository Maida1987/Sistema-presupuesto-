...
| **2 — Importación y precios** ✅ (⚠️ fórmula de precios sin confirmar con el negocio, ver más abajo) | Import de Excel (mapeo, preview, calidad de datos), `price_lists`/`price_list_items`, `pricing_rules` y `price_history` con la fórmula documentada en `04-importacion-y-precios.md` | Fase 1 | Importación E2E de un Excel real de cada formato de ejemplo, comparador de precios funcionando |
...
| **6 — Auditoría, dashboard y buscador global** ✅ | Consulta de `audit_logs` (`GET /audit-logs` con filtros por módulo/entidad/usuario, pantalla con tabla y filtros), dashboard con indicadores reales (saldo total, cuentas activas, remitos por liquidar, productos/proveedores/listas) y alertas (listas desactualizadas, productos sin precio, remitos sin firmar, clientes con saldo elevado, variaciones de precio significativas), buscador global (clientes/proveedores/productos/remitos) | Fases 1–5 | Alertas reales sobre datos de prueba, auditoría consultable por entidad ✅ |
| **7 — Reportes y exportaciones** ✅ | Reportes de clientes/productos/remitos (Excel/CSV) y extracto de cuenta corriente por cliente (PDF/Excel/CSV, PDF paginado sin truncar) | Fase 6 | Exportaciones verificadas contra los datos de origen ✅ |
